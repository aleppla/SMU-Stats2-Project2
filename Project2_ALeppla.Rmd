...
bank2=bank[,c(12:14,16:20,21)]
# age is the only factor that doesn't improve the model
...
bank2_train=bank_train[,c(12:14,16:20,21)]
bank2_test=bank_test[,c(12:14,16:20,21)]
...
#glmnet requires a matrix 
dat.train.x <- model.matrix(y~.^2-1,bank_train[,c(-4,-5,-10,-11,-13,-17,-19,-20)])
...
#Run model from training set on valid set 
dat.test.x = model.matrix(y~.^2-1,bank_test[,c(-4,-5,-10,-11,-13,-17,-19,-20)])
...
model.main2<-glm(y~job+marital+education+contact+month+campaign+poutcome+emp.var.rate+cons.conf.idx + 
  job*cons.conf.idx + contact*cons.conf.idx + month*emp.var.rate + month*cons.conf.idx + campaign*cons.conf.idx +
  poutcome*emp.var.rate + poutcome*cons.conf.idx, bank_train,family = binomial(link="logit"))

cor(bank[,c(12,)])

#housing +loan + job*marital + job*housing + job*poutcome + marital*month + marital*poutcome + contact*month + previous*poutcome + month*campaign +  
...
hoslem.test(model.main$y, fitted(model.main2), g=10)
# Sig. lack of fit, but large n

fit1.pred.train <- predict(model.main2, newdata = bank_train) 
...
#Run model from training set on valid set 
fit1.pred.test <- predict(model.main2, newdata = bank_test)
